...
function QW_ACT_R17(elements, results, nodes, rule) {
    if (rule.metadata.outcome === 'passed') {
        ...
        addToElements(elements, 'img_01b', (rule.results.filter((r) => r.verdict !== 'inapplicable')).length);
    }
    ...
    if (imgEmptyAlt.length > 0) {
        ...
        addToElements(elements, 'img_02', (rule.results.filter((r) => r.verdict !== 'inapplicable')).length);
    }
}
...
function QW_ACT_R19(elements, results, nodes, rule) {
    if (rule.metadata.outcome === 'failed') {
        ...
        addToElements(elements, 'iframe_01', (rule.results.filter((r) => r.verdict !== 'inapplicable')).length);
    }
}
