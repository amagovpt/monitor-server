...
function QW_ACT_R1(elements, results, nodes, rule) {
    if (rule.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'titleOk', rule.results.filter((r) => r.verdict === 'passed'));
    }
}
function QW_ACT_R2(elements, results, nodes, rule) {
    if (rule.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'langNo', rule.results.filter((r) => r.verdict === 'failed'));
    }
    else if (rule.metadata.outcome === 'passed') {
        addToElements(elements, 'lang', rule.metadata.passed);
        addToResults(results, 'lang_01');
        addToNodes(nodes, 'lang', rule.results.filter((r) => r.verdict === 'passed'));
    }
}
function QW_ACT_R3(elements, results, nodes, rule) {
    if (rule.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'langMatchNo', rule.results.filter((r) => r.verdict === 'failed'));
    }
}
...
function QW_ACT_R5(elements, results, nodes, rule) {
    if (rule.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'langCodeNo', rule.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_ACT_R6(elements, results, nodes, rule) {
    if (rule.metadata.outcome === 'passed') {
        ...
        addToNodes(nodes, 'inpImgAltNo', rule.results.filter((r) => r.verdict === 'passed'));
    }
        ...
        addToNodes(nodes, 'inpImgAltNo', rule.results.filter((r) => r.verdict === 'failed'));
        addToElements(elements, 'inp_img_01b', (rule.results.filter((r) => r.verdict !== 'inapplicable')).length);
    }
}
function QW_ACT_R9(elements, results, nodes, rule) {
    if (rule.metadata.outcome === 'warning') {
        ...
        addToNodes(nodes, 'aSameText', rule.results.filter((r) => r.verdict === 'warning'));
    }
}
function QW_ACT_R16(elements, results, nodes, rule) {
    if (rule.metadata.outcome === 'passed') {
        ...
        addToNodes(nodes, 'inputLabel', rule.results.filter((r) => r.verdict === 'passed'));
    }
    else if (rule.metadata.outcome === 'failed') {
        addToElements(elements, 'inputLabelNo', rule.metadata.failed);
        addToResults(results, 'input_02');
        addToNodes(nodes, 'inputLabelNo', rule.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_ACT_R17(elements, results, nodes, rule) {
    if (rule.metadata.outcome === 'passed') {
        ...
        addToNodes(nodes, 'img', rule.results.filter((r) => r.verdict === 'passed'));
    }
    else if (rule.metadata.outcome === 'failed') {
        addToElements(elements, 'imgAltNo', rule.metadata.failed);
        addToResults(results, 'img_01b');
        addToNodes(nodes, 'imgAltNo', rule.results.filter((r) => r.verdict === 'failed'));
    }
    const imgEmptyAlt = rule.results.filter((r) => r.resultCode === 'RC1');
...
function QW_ACT_R19(elements, results, nodes, rule) {
    if (rule.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'iframeTitleNo', rule.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_ACT_R37(elements, results, nodes, rule) {
    if (rule.metadata.outcome === 'failed') {
        addToElements(elements, 'colorContrast', rule.metadata.failed);
        addToResults(results, 'color_02');
        addToNodes(nodes, 'colorContrast', rule.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_ACT_R47(elements, results, nodes, rule) {
    if (rule.metadata.outcome === 'failed') {
        addToElements(elements, 'lineHeightNo', rule.metadata.failed);
        addToResults(results, 'css_01');
        addToNodes(nodes, 'lineHeightNo', rule.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_HTML_T1(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'areaAltNo', technique.results.filter((r) => r.verdict === 'failed'));
    }
        ...
        addToNodes(nodes, 'areaAltNo', technique.results.filter((r) => (r.verdict === 'passed' || r.verdict === 'warning')));
    }
}
function QW_HTML_T2(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'tableDataCaption', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
...
function QW_HTML_T6(elements, results, nodes, technique) {
    const passed = technique.results.filter((r) => r.verdict === 'passed');
    if (passed.length > 0) {
        addToElements(elements, 'ehandBoth', passed.length);
        addToResults(results, 'ehandler_03');
        addToNodes(nodes, 'ehandBoth', passed);
    }
    const failed = technique.results.filter((r) => r.verdict === 'failed');
    if (failed.length > 0) {
...
function QW_HTML_T7(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'abbrNo', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_HTML_T8(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'imgAltNot', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
...
function QW_HTML_T17(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'tableComplexError', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_HTML_T19(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'linkRel', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_HTML_T20(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'w3cValidatorErrors', technique.results.filter((r) => r.verdict === 'failed'));
    }
        ...
        addToNodes(nodes, 'w3cValidatorErrors', technique.results.filter((r) => r.verdict === 'passed'));
    }
}
...
function QW_HTML_T28(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'liNoList', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_HTML_T30(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'tableLayout', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_HTML_T32(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'passed') {
        ...
        addToNodes(nodes, 'formSubmitNo', technique.results.filter((r) => r.verdict === 'passed'));
    }
        ...
        addToNodes(nodes, 'formSubmitNo', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
...
function QW_HTML_T34(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'aImgAltNo', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
...
function QW_HTML_T38(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'warning') {
        ...
        addToNodes(nodes, 'aSkipFirst', technique.results.filter((r) => r.verdict === 'warning'));
    }
    else if (technique.metadata.outcome === 'failed') {
        addToElements(elements, 'aSkipFirst', undefined);
        addToResults(results, 'a_01b');
        addToNodes(nodes, 'aSkipFirst', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_HTML_T40(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'focusBlur', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_HTML_T41(elements, results, nodes, technique) {
    const incorrectScope = technique.results.filter((r) => r.resultCode === 'RC2' || r.resultCode === 'RC4');
...
function QW_HTML_T42(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'ehandTagNo', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_HTML_T43(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'justifiedTxt', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_CSS_T1(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'fontAbsVal', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
...
function QW_CSS_T7(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'colorFgBgNo', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_BP1(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'hx', technique.results.filter((r) => r.verdict === 'warning'));
    }
}
function QW_BP2(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'imgAltLong', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_BP4(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'aGroupNo', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_BP5(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'tableNested', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_BP6(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'titleLong', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_BP7(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'titleChars', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_BP8(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'hxNo', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_BP9(elements, results, nodes, technique) {
    if (technique.metadata.passed > 0) {
        ...
        addToNodes(nodes, 'tableLayoutCaption', technique.results.filter((r) => r.verdict === 'passed'));
    }
}
function QW_BP10(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'layoutElem', technique.results.filter((r) => r.verdict === 'failed'));
    }
    else if (technique.metadata.outcome === 'passed') {
...
function QW_BP11(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'brSec', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_BP13(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'aAdjacentSame', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
function QW_BP14(elements, results, nodes, technique) {
    if (technique.metadata.outcome === 'failed') {
        ...
        addToNodes(nodes, 'layoutFixed', technique.results.filter((r) => r.verdict === 'failed'));
    }
}
...
function addToNodes(nodes, key, elements) {
    nodes[key] = elements;
}
