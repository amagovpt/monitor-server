...
class ListTags {
    constructor(tags) {
        this.tags = tags;
        this.nEntities = tags.reduce((n, t) => {
        ...
        for (const tag of tags || []) {
            ...
            this.AA += tag.AA;
            this.AAA += tag.AAA;
            this.frequencies = this.frequencies.map((v, j) => {
    ...
    getTopFiveErrors() {
        ...
        return lodash_orderby_1.default(errors, ["n_occurrences", "n_pages", "n_websites"], ["desc", "desc", "desc"]).slice(0, 5);
    }
    getTopFiveBestPractices() {
        ...
        return lodash_orderby_1.default(practices, ["n_occurrences", "n_pages", "n_websites"], ["desc", "desc", "desc"]).slice(0, 5);
    }
    getPassedAndWarningOccurrenceByTag(test) {
        const occurrences = new Array();
        for (const tag of this.tags || []) {
            if (tag.success[test] && tests_1.default[test]["result"] !== "failed") {
                occurrences.push(tag.success[test]["n_occurrences"]);
            }
        }
        return occurrences;
    }
    getErrorOccurrenceByTag(test) {
        const occurrences = new Array();
        for (const tag of this.tags || []) {
            if (tag.errors[test] && tests_1.default[test]["result"] === "failed") {
