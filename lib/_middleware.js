...
const { getElementsMapping } = require('./_mapping');
...
function map_elems(evaluation) {
  ...
  for (const assertion of evaluation.graph[0].assertions || []) {
    const test = assertion.test['@id'];
    const outcome = assertion.result.source[0].result.outcome;
...
/*function map_nodes(report, results) {
  const tests = require('./tests.json');
  //const mapping = require('./rule-mapping.json');
  ...
  for (const result in results || {}) {
    if (xpath[result]) {
      nodes[result] = xpath[result];
    } else {
      if (!nodes[result]) {
        nodes[result] = convert_css_selector_to_xpath(assertion.result.source[0].result.pointer);
      } else {
        nodes[result] += '|' + convert_css_selector_to_xpath(assertion.result.source[0].result.pointer);
      }
    }
...
function parse_evaluation(evaluation) {
  try {
    ...
    report.pagecode = evaluation.report.system.dom.processed.html.plain;
    report.data = {};
    report.data.title = evaluation.report.system.dom.processed.title;
    report.data.score = generate_score(evaluation.earlReport);
    report.data.rawUrl = evaluation.earlReport.graph[0].source;
    const { elements, results, nodes } = getElementsMapping(evaluation.report);
    report.data.elems = _.clone(elements); //map_elems(evaluation.earlReport);
    report.data.nodes = _.clone(nodes); //map_nodes(results);
    ...
    report.data.tot.info.htmlTags = evaluation.report.system.dom.processed.elementCount; //count_html_tags(evaluation.postProcessingHTML);
    report.data.tot.info.size = encodeURI(report.pagecode).split(/%..|./).length - 1;
    report.data.tot.info.cssRules = calculate_css_rules(evaluation.report);
    report.data.tot.info.encoding = 'utf-8';
    report.data.tot.info.lang = get_html_lang(evaluation.report.system.dom.processed.html.plain);
    report.data.tot.info.content = 'text/html';
    report.data.tot.info.hash = generate_hash(report.data.date);
    report.data.tot.info.tests = _.size(evaluation.earlReport.graph[0].assertions);
    report.data.tot.elems = _.clone(report.data.elems);
    report.data.tot.results =  _.clone(results); // map_results(evaluation.earlReport);
