...
function _process(tot, pagecode, nodes, url) {
  ...
  for (let ee in tot.results) {
    ...
    let msg = ee; //formatTestText(testsResults[ee], cant);
    ...
    if (!_.includes(hidden, ele)) {
      result["tech_list"].push(testView2(ele, /*elems[ele]*/ele, tot['elems'][ele], ee, url));
      if (!isNaN(tot['elems'][ele])) {
        result["tnum"] += parseInt(tot['elems'][ele]);
      }
    }
    
    result["tech_list"].push(testView2(tes, /*elems[tes]*/tes, tnum, ee, url));
    ...
    data.tableTest.push({'class': _class, 'level': _.toUpper(lev), 'sc': sctable, 'desc': msg, 'tnum': result['tnum'], 'prio': prio});
