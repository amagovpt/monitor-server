  ...
  return new Promise((resolve, reject) => {
    readFile(SECRET_KEY_FILE, (err, password) => {
      if (err) {
        ...
        const cipher = createCipher('aes-256-ctr', _.trim(_.toString(password)));
  ...
  return new Promise((resolve, reject) => {
    readFile(SECRET_KEY_FILE, (err, password) => {
      if (err) {
        ...
        const decipher = createDecipher('aes-256-ctr', _.trim(_.toString(password)));
...
module.exports.generate_hash = (content) => {
