...
export class PageParser {
  ...
  public verifyAccessiblityPossibleStatement(url: string): boolean {
    return (
      url.includes("/acessibilidade") ||
      this.verifyAccessiblityPossibleStatementTitle()
    );
  }
  ...
  public processProcedure(procedure) {
    ...
    return listElements.flatMap((li) => {
      if (li.name === "li") return this.processProcedureElement(data, li);
      else return [];
    });
  }
  ...
  private getOtherProcedureData(element, data) {
    ...
    listElements.flatMap((li) => {
      ...
      if (text && text.trim()) {
        ...
        if (attName === "Amostra") content = content.replace(/[^\d]/g, "");
  ...
  private getText(element: any): string {
    ...
    let text = "";
    if (element.type === "text") text = text + element.data;
    children?.map((child) => {
  ...
  private getTextRecursion(child) {
    let text = "";
    if (child.type === "text") text = text + child.data;
  ...
  public getContacts() {
    ...
    children?.map((element, index) => {
      ...
      if (name === "dd") {
        ...
        contacts.push({
          contactType: previousElementText,
          contact: elementText,
        });
      }
    });
