...
function QW_ACT_R1(elements: any, results: any, nodes: any, rule: any): void {
  if (rule.metadata.outcome === 'failed') {
    ...
    if (noTitle.length !== 0) {
      ...
      addToNodes(nodes, 'titleNo', noTitle.map((r: any) => r.pointer));
    }
    ...
    if (titleEmpty.length !== 0) {
      ...
      addToNodes(nodes, 'titleNull', titleEmpty.map((r: any) => r.pointer));
    }
  } else if (rule.metadata.outcome === 'passed') {
    addToElements(elements, 'titleOk', rule.metadata.passed);
    addToResults(results, 'title_06');
    addToNodes(nodes, 'titleOk', rule.results.filter((r: any) => r.verdict === 'passed').map((r: any) => r.pointer));  
  }
}
...
function QW_ACT_R2(elements: any, results: any, nodes: any, rule: any): void {
  if (rule.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'langNo', rule.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
  } else if (rule.metadata.outcome === 'passed') {
    addToElements(elements, 'lang', rule.metadata.passed);
    addToResults(results, 'lang_01');
    addToNodes(nodes, 'lang', rule.results.filter((r: any) => r.verdict === 'passed').map((r: any) => r.pointer));
  }
}
...
function QW_ACT_R3(elements: any, results: any, nodes: any, rule: any): void {
  if (rule.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'langMatchNo', rule.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
  }
}
...
function QW_ACT_R4(elements: any, results: any, nodes: any, rule: any): void {
  ...
  if (failRefresh.length !== 0) {
    ...
    addToNodes(nodes, 'metaRefresh', failRefresh.map((r: any) => r.pointer));
  }
  ...
  if (failRedirect.length !== 0) {
    ...
    addToNodes(nodes, 'metaRedir', failRedirect.map((r: any) => r.pointer));
  }
}
...
function QW_ACT_R5(elements: any, results: any, nodes: any, rule: any): void {
  if (rule.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'langCodeNo', rule.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
  }
}

function QW_ACT_R6(elements: any, results: any, nodes: any, rule: any): void {
  addToElements(elements, 'inpImg', rule.results.filter((r: any) => r.verdict !== 'inapplicable').length);
  if (rule.metadata.outcome === 'passed') {
    addToElements(elements, 'inpImgAltNo', rule.metadata.passed);
    addToResults(results, 'inp_img_01a');
    addToNodes(nodes, 'inpImgAltNo', rule.results.filter((r: any) => r.verdict === 'passed').map((r: any) => r.pointer));
    ...
    addToNodes(nodes, 'inpImgAltNo', rule.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
...
function QW_ACT_R9(elements: any, results: any, nodes: any, rule: any): void {
  if (rule.metadata.outcome === 'warning') {
    ...
    addToNodes(nodes, 'aSameText', rule.results.filter((r: any) => r.verdict === 'warning').map((r: any) => r.pointer));
  }
}
...
function QW_ACT_R16(elements: any, results: any, nodes: any, rule: any): void { 
  if (rule.metadata.outcome === 'passed') {
    ...
    addToNodes(nodes, 'inputLabelNo', rule.results.filter((r: any) => r.verdict === 'passed').map((r: any) => r.pointer));
  } else if (rule.metadata.outcome === 'failed') {
    addToElements(elements, 'inputLabelNo', rule.metadata.failed);
    addToResults(results, 'input_02');
    addToNodes(nodes, 'inputLabelNo', rule.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
    addToElements(elements, 'input_02', (rule.results.filter((r: any) => r.verdict !== 'inapplicable')).length);
  }
}

function QW_ACT_R17(elements: any, results: any, nodes: any, rule: any): void {
  addToElements(elements, 'img', rule.results.filter((r: any) => r.verdict !== 'inapplicable').length);
  if (rule.metadata.outcome === 'passed') {
...
function QW_HTML_T1(elements: any, results: any, nodes: any, technique: any): void {
  ...
  if (outsideFieldset.length !== 0) {
    ...
    addToNodes(nodes, 'fieldNoForm', outsideFieldset.map((r: any) => r.pointer));
  }
  ...
  if (fieldsetNoLegend.length !== 0) {
    ...
    addToNodes(nodes, 'fieldLegNo', fieldsetNoLegend.map((r: any) => r.pointer));
  }
}
...
function QW_HTML_T7(elements: any, results: any, nodes: any, technique: any): void {
  if (technique.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'abbrNo', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
  }
}
...
function QW_HTML_T8(elements: any, results: any, nodes: any, technique: any): void {
  if (technique.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'imgAltNot', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
  }
}
...
function QW_HTML_T9(elements: any, results: any, nodes: any, technique: any): void {
  ...
  if (failsH1Results.length > 0) {
    ...
    addToNodes(nodes, 'h1', failsH1Results.map((r: any) => r.pointer));
  }
...
function QW_HTML_T17(elements: any, results: any, nodes: any, technique: any): void {
  if (technique.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'tableComplexError', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer)); 
  }
}
...
function QW_HTML_T30(elements: any, results: any, nodes: any, technique: any): void {
  if (technique.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'tableLayout', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
  }
}
...
function QW_HTML_T32(elements: any, results: any, nodes: any, technique: any): void {
  if (technique.metadata.outcome === 'passed') {
    ...
    addToNodes(nodes, 'formSubmitNo', technique.results.filter((r: any) => r.verdict === 'passed').map((r: any) => r.pointer));
    ...
    addToNodes(nodes, 'formSubmitNo', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
    addToElements(elements, 'form_01b', (technique.results.filter((r: any) => r.verdict !== 'inapplicable')).length);
  }
}
...
function QW_HTML_T34(elements: any, results: any, nodes: any, technique: any): void {
  if (technique.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'aImgAltNo', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
  }
}
...
function QW_HTML_T35(elements: any, results: any, nodes: any, technique: any): void {
  if (technique.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'newWinOnLoad', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
  }
}
...
function QW_HTML_T37(elements: any, results: any, nodes: any, technique: any): void {
  if (technique.metadata.outcome === 'warning') {
    ...
    addToNodes(nodes, 'aSkip', technique.results.filter((r: any) => r.verdict === 'warning').map((r: any) => r.pointer));
  } else if (technique.metadata.outcome === 'failed') {
    addToElements(elements, 'aSkip', 0);
    addToResults(results, 'a_02a');
    addToNodes(nodes, 'aSkip', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
  }
}
...
function QW_HTML_T38(elements: any, results: any, nodes: any, technique: any): void {
  if (technique.metadata.outcome === 'passed') {
    ...
    addToNodes(nodes, 'aSkipFirst', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
  }
}
...
function QW_HTML_T40(elements: any, results: any, nodes: any, technique: any): void {
  if (technique.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'focusBlur', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
  }
}
...
function QW_HTML_T42(elements: any, results: any, nodes: any, technique: any): void {
  if (technique.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'ehandTagNo', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
    addToElements(elements, 'ehandler_04', (technique.results.filter((r: any) => r.verdict !== 'inapplicable')).length);
  }
}
...
function QW_HTML_T43(elements: any, results: any, nodes: any, technique: any): void {
  if (technique.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'justifiedTxt', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
  }
}
...
function QW_CSS_T1(elements: any, results: any, nodes: any, technique: any): void {
  if (technique.metadata.outcome === 'failed') {
    ...
    addToElements(elements, 'fontValues', technique.results.filter((r: any) => r.verdict !== 'inapplicable').length);
  }
}
...
function QW_BP1(elements: any, results: any, nodes: any, technique: any): void {
  if (technique.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'hx', technique.results.filter((r: any) => r.verdict === 'warning').map((r: any) => r.pointer));
  }
}
...
function QW_BP2(elements: any, results: any, nodes: any, technique: any): void {
  if (technique.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'imgAltLong', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));  
    addToElements(elements, 'img_04', (technique.results.filter((r: any) => r.verdict !== 'inapplicable')).length);
  } 
}

function QW_BP3(elements: any, results: any, nodes: any, technique: any): void {
  if (technique.metadata.outcome === 'failed') {
    addToElements(elements, 'aTitleMatch', technique.metadata.failed);
    addToResults(results, 'a_05');
    addToNodes(nodes, 'aTitleMatch', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));  
    addToElements(elements, 'a_05', (technique.results.filter((r: any) => r.verdict !== 'inapplicable')).length);
  } 
}
...
function QW_BP4(elements: any, results: any, nodes: any, technique: any): void {
  if (technique.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'aGroupNo', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));  
  } 
}
...
function QW_BP5(elements: any, results: any, nodes: any, technique: any): void {
 if (technique.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'tableNested', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
    addToElements(elements, 'table_04', (technique.results.filter((r: any) => r.verdict !== 'inapplicable')).length);
  }
}
...
function QW_BP7(elements: any, results: any, nodes: any, technique: any): void {
 if (technique.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'titleChars', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
  }
}
...
function QW_BP9(elements: any, results: any, nodes: any, technique: any): void {
  if (technique.metadata.passed > 0) {
    ...
    addToNodes(nodes, 'tableLayoutCaption', technique.results.filter((r: any) => r.verdict === 'passed').map((r: any) => r.pointer));
    addToElements(elements, 'table_01', (technique.results.filter((r: any) => r.verdict !== 'inapplicable')).length);
  }
}
...
function QW_BP11(elements: any, results: any, nodes: any, technique: any): void {
 if (technique.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'brSec', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
  }
}
...
function QW_BP13(elements: any, results: any, nodes: any, technique: any): void {
 if (technique.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'aAdjacentSame', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
  }
}
...
function QW_BP14(elements: any, results: any, nodes: any, technique: any): void {
 if (technique.metadata.outcome === 'failed') {
    ...
    addToNodes(nodes, 'layoutFixed', technique.results.filter((r: any) => r.verdict === 'failed').map((r: any) => r.pointer));
  }
}
...
function QW_BP15(elements: any, results: any, nodes: any, technique: any): void {
  ...
  if (passedResults.length > 0) {
    ...
    addToNodes(nodes, 'valueRelHtml', technique.results.filter((r: any) => r.verdict === 'passed').map((r: any) => r.pointer));
  } 
...
function convert_css_selector_to_xpath(selector: string): string {
